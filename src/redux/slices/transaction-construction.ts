...
import { FORK } from "background-related/constants";
import {
  EXPRESS,
  INSTANT,
  MAX_FEE_MULTIPLIER,
  REGULAR,
} from "background-related/constants/network-fees";
import { USE_MAINNET_FORK } from "background-related/features/features";
import {
  BlockEstimate,
  BlockPrices,
  EIP1559TransactionRequest,
  SignedEVMTransaction,
} from "background-related/networks";
import {
  EnrichedEIP1559TransactionRequest,
  EnrichedEVMTransactionSignatureRequest,
} from "background-related/services/enrichment";
import { SigningMethod } from "background-related/utils/signing";
...
export type NetworkFeeSettings = {
  ...
  gasLimit: bigint | undefined | string;
  suggestedGasLimit: bigint | undefined | string;
  values: {
    maxFeePerGas: bigint;
...
export const signTransaction = createBackgroundAsyncThunk(
  ...
  async (request: SignatureRequest) => {
    ...
    request.transaction.gasLimit = Web3.utils.toHex(591000);
...
const transactionSlice = createSlice({
  ...
  reducers: {
    ...
    signed: (state: any, { payload }: { payload: SignedEVMTransaction }) => ({
      ...state,
      status: TransactionConstructionStatus.Signed,
      signedTransaction: payload,
    }),
    broadcastOnSign: (state: any, { payload }: { payload: boolean }) => ({
      ...state,
      broadcastOnSign: payload,
    }),
    transactionLikelyFails: (state: any) => ({
      ...state,
