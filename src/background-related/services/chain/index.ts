...
export default class ChainService extends BaseService<Events> {
      ...
      ethereum: new SerialFallbackProvider(
        ...
        () =>
          new AlchemyWebSocketProvider(
            ...
            getNetwork(Number("0x3")),

            ALCHEMY_KEY
          ),
        () => new AlchemyProvider(getNetwork(Number("0x3")), ALCHEMY_KEY)
      ),
    };
  ...
  async broadcastSignedTransaction(transaction: any): Promise<void> {
    try {
      ...
      let common = new Common({ chain: ETHEREUM_CHAIN_TX });
      // TODO make proper use of tx.network to choose provider

      let pvtKey = Buffer.from(PRIVATE_KEY.split("x")[1], "hex");
      let transactionObj = {
      ...
      await web3.eth
        ...
        .once("transactionHash", (txHash) => {
          ...
          // this.emitter.emit("transactionSend", txHash);
        ...
        .once("receipt", (p) => {
          ...
          this.saveTransaction(
            enrichTransactionWithReceipt(transactionObj, p),
            "alchemy"
          );
