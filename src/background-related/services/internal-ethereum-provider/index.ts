...
export default class InternalEthereumProviderService extends BaseService<Events> {
  ...
  async routeSafeRPCRequest(
    ...
    switch (method) {
      ...
      case "eth_chainId":
        console.log(
          "CHAINNNNNNNNNNNNNNNNNNN",
          this.chainService.ethereumNetwork.chainID,
          BigInt(this.chainService.ethereumNetwork.chainID).toString(16)
        );
      ...
      case "eth_sendTransaction":
        return this.signTransaction(
          ...
          const res = await this.chainService.broadcastSignedTransaction(
            signed
          );
          return res;
        });
  ...
  private async signTransaction(transactionRequest: JsonRpcTransactionRequest) {
    console.log("TR REQUEST==============", transactionRequest);
    ...
    console.log("FROM===============", from, convertedRequest);
    // convertedRequest.gasLimit = BigInt(web3.utils.toHex(591000));
  ...
  private async signTypedData(params: SignTypedDataRequest) {
    return new Promise<string>((resolve, reject) => {
      localStorage.setItem("signTypedData", JSON.stringify(params));
