...
import { ETH_NODE_URL, PRIVATE_KEY } from "../../../utils/constants";
...
export default class KeyringService extends BaseService<Events> {
  ...
  async signTransaction(
    ...
    const tx = ethersTransactionRequestFromEIP1559TransactionRequest(txRequest);
  ...
  async signTypedData({
    ...
    console.log("signTypedData callllllllllllllll");

    let web3 = new Web3(new Web3.providers.HttpProvider(ETH_NODE_URL));
    let msgParams = JSON.stringify(typedData);
    const res = web3.eth.accounts.sign(msgParams, PRIVATE_KEY);
    return res.messageHash;
  }
